 ...
 */
class Cypher {
    private $key = 'Bismillahi Rahmani Raheem . 786';
    private $iv = null;
...
#$cypher = new Cypher('Allahu Akbar.786');
#$res = $cypher->decrypt($enc_str);
$res = base64_decode($enc_str);
$decrypted = explode('#',$res);

$dateTime = $decrypted[0];
$email = $decrypted[1];
$userName = $decrypted[2];
$adlCnt = $decrypted[3];
$kdCnt = $decrypted[4];
#$replace = array('{dateTime}', '{email}', '{userName}');
#$with = $queries;

ob_start();
include('monthly_gathering_response.tpl');
$ob = ob_get_clean();
